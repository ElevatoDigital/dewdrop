...
class SelectSort extends HelperAbstract implements SelectModifierInterface
{
    ...
    public function modifySelect(Fields $fields, Select $select)
    {
        ...
        $sort                  = $this->listingHandler->getSortFromRequest($this->getPrefix());
        $sorts                 = $sort['sorts'];
        $dirs                  = $sort['dirs'];
     ...
     * @return array Id
     */
    public function getDefaultSorting(Fields $fields)
    {
    ...
    {
        $sorting = $this->getDefaultSorting($fields);

        if (!empty($sorting)) {
            $field = $sorting['field'];

            $this->sortedFields[$field->getQueryStringId()] = $this->defaultDirection;
